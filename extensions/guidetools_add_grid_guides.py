...
class Grid_Guides(inkex.Effect):
    def __init__(self):
        ...
        self.OptionParser.add_option(
            '--include_center_col_guides',
            action='store',
            type='inkbool',
            dest='include_center_col_guides',
            default=False,
            help='Include centered guides inside gutters)')

        self.OptionParser.add_option(
            '--include_outer_col_gutter',
            action='store',
            type='inkbool',
            dest='include_outer_col_gutter',
            default=True,
        ...
        self.OptionParser.add_option(
            '--include_outer_row_gutter',
            action='store',
            type='inkbool',
            dest='include_outer_row_gutter',
            default=True,
            help='Include outer gutters (double guides)')

        self.OptionParser.add_option(
            '--include_center_row_guides',
            action='store',
            type='inkbool',
            dest='include_center_row_guides',
            default=True,
            help='Include centered guides inside gutters')
    ...
    def effect(self):
        ...
        if (tab == "\"columns\""):
            ...
            if (col_alignment == 'centered'):
                hor_start = round(canvas_width / 2) - round(
                    total_col_width / 2) + col_offset

            if (col_alignment == 'right'):
                hor_start = canvas_width - total_col_width + col_offset

            # Create column guides with column_spacings
            guidetools.drawDoubleGuides(cols, col_width, col_gut, hor_start,
                                        has_outer_col_gutter, "vertical",
                                        namedview)
            ...
            if col_gut > 0 and has_center_col_guides == True:
                guidetools.drawDoubleGuides(
                    center_cols, col_width + col_gut, 0, center_hor_start,
            ...
            if (delete_hor):
                guidetools.deleteGuidesByOrientation(self.document,
                                                     'horizontal')
            ...
            if (row_alignment == 'top'):
                vert_start = round(
                    canvas_height) - total_row_height - row_offset

            if (row_alignment == 'centered'):
                vert_start = round(canvas_height / 2) - round(
                    total_row_height / 2) - row_offset

            if (row_alignment == 'bottom'):
                vert_start = -row_offset

            # Create row guides
            guidetools.drawDoubleGuides(rows, row_height, row_gut, vert_start,
                                        has_outer_row_gutter, "horizontal",
                                        namedview)
            ...
            if row_gut > 0 and has_center_row_guides == True:
                guidetools.drawDoubleGuides(
                    center_rows, row_height + row_gut, 0, center_vert_start,

...
class remove_selected_guides(inkex.Effect):
    ...
    def effect(self):
        ...
        for node in self.document.xpath(
                "//sodipodi:guide", namespaces=inkex.NSS):
            if remove_hor_guide and (node.get('orientation') == '0,1'):
                node.getparent().remove(node)
            if remove_vert_guide and (node.get('orientation') == '1,0'):
                node.getparent().remove(node)
            if remove_ang_guide and (node.get('orientation') != '0,1') and (
                    node.get('orientation') != '1,0'):
                node.getparent().remove(node)
